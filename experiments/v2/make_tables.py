"""Generate every LaTeX table in the manuscript from processed results.
...
def t1_instances():
    ...
    w("tab_instances.tex",
      ...
      "best-known $\\langle C\\rangle$ & ratio & starts at best \\\\\n\\midrule\n"
      + "\n".join(rows) + "\n\\bottomrule\n\\end{tabular}\n")
...
def t3_f2():
    ...
    for _,r in f2.sort_values(["noise","budget","method"]).iterrows():
        rows.append(f"{NL[r.noise]} & {int(r.budget)} & {LABEL[r.method]} & {r.mean_err_none:.4f} & "
                    ...
                    f"{r.effect_in_ratio_units:+.4f} & {r['p_adj_BH']:.2e} & {r.verdict} \\\\")
    w("tab_f2.tex",
...
def macros():
    ...
    for _,r in f1.iterrows():
        ...
        m("DisAP"+tag, f"{r['p_adj_BH']:.1e}")
    m("FONemaxP", f"{f1['p_adj_BH'].max():.1e}")
