...
\title{Two Integration Defects in a Zero-Noise-Extrapolation\\
Pipeline, and Sampling-Free Oracles that Catch Them}
...
\FONeminDis--\FONemaxDis{} of $C_{\max}$, and the method selected as best changes in
\FlipRateAllPct\% of cells among \WinnerScope. Selecting on one evaluation sample and paying on an
independent one, the defect costs \SplitRegretExcess{} of $C_{\max}$ in excess regret. We verify
...
\begin{table}[t]
\caption{Estimator bias and sampling variance reported \emph{separately} from differences of absolute
errors, normalised by $C_{\max}$.}
\label{tab:bv}
...
\begin{table}[t]
\caption{Method selection. \emph{Scope} names the candidate set. \emph{Near-tie rate} is the fraction
of cells whose best and second-best candidates differ by less than $0.01\,C_{\max}$.
\emph{Split-sample regret} selects on one evaluation replicate and pays on an independent one.}
\label{tab:winner}
\centering\footnotesize
