...
overstates the substantive effect: \NearTiePct\% of cells are near ties. Selecting on one
evaluation sample and paying on an independent one, the defect costs \SplitRegretExcess{} of
$C_{\max}$ in excess regret --- small in absolute terms, and concentrated on the gate-dominated
snapshot. We verify
...
and corrected implementations under common random numbers, from a fresh seed namespace on a clean
committed tree. Differences are normalised by each instance's own $C_{\max}$ before averaging.
...
\begin{table*}[t]
\caption{Method selection. Two candidate scopes are reported: the three mitigated methods, and the
same set with the unmitigated option added. \emph{Near-tie} is the fraction of cells whose best and
second-best candidates differ by less than $0.01\,C_{\max}$ --- it is large, so a changed winner
often reflects a near-tie rather than a substantive ranking change. \emph{Split-sample regret}
selects the method on one evaluation replicate and measures the consequence on an independent one;
the same-sample minimum is never described as ``best attainable''.}
\label{tab:winner}
\centering\small
\input{tables/tab_winner}
\end{table*}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{../figures/figR2_impact}
\caption{Implementation contrast on the fresh confirmation data. (a) Per-cell estimation error,
defective versus corrected, paired under common random numbers. (b) Mean normalised difference with
stratified (solid) and cluster (light) intervals. (c) Split-sample regret. Simulator only.}
\label{fig:impact}
\end{figure*}

\textbf{Selection changes, with scope stated.} Table~\ref{tab:winner} reports the rate at which the
selected method changes, for the mitigated-only candidate set and for the set including the
unmitigated option, together with the near-tie rate. A substantial share of cells are near ties, so
a changed winner does not by itself imply a substantive ranking change --- which is why we report
regret on an independent sample rather than a same-sample minimum. We do not describe any
same-sample minimum as ``best attainable''.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{../figures/figR2_perinstance}
