...
method selected as best changes in \FlipRateAllPct\% of cells among \WinnerScope. That rate is
not evidence of a substantive ranking change: once compilation is held fixed,
\TiePracticalAllPct\% of cells are practical near ties and \TieStatAllPct\% are statistically
indistinguishable. Our primary endpoint is the paired held-out loss difference between the
legacy-selected and corrected-selected method on an independent evaluation replicate. It is
\HeldOutDelta{} of $C_{\max}$ (95\% CI [\HeldOutLo, \HeldOutHi]) --- resolvable, but roughly an
order of magnitude below our declared practical threshold of $0.01$, and not resolvable at all on
the readout-dominated device. \textbf{The defect reliably changes which method a benchmark selects,
yet its downstream decision cost in these conditions is negligible.} We verify
...
\noindent We also claim none of the following, all established: that folding leaves state-preparation
...
endpoint and Table~\ref{tab:ties} the near-tie structure. Two findings must be read together.

First, the selected method changes often: \FlipRateAllPct\% of cells, and the rate is unchanged
when the unmitigated option is added to the candidate set. Second, that rate carries almost no
decision weight here. With compilation held fixed across arms, \TiePracticalAllPct\% of cells have
a top-two gap below the practical threshold and \TieStatAllPct\% are within the Monte-Carlo scale
of the selection replicate; the median gap is under $0.002\,C_{\max}$. The held-out cost of
following the defective selection is \HeldOutDelta{} of $C_{\max}$ (stratified CI
[\HeldOutLo, \HeldOutHi]; cluster CI [\HeldOutClusterLo, \HeldOutClusterHi]) --- statistically
resolvable, but about an order of magnitude below the threshold we declared in advance, and on
\code{dev\_lagos} the interval crosses zero.

\textbf{We therefore do not claim the defect materially degrades method selection in these
conditions.} It reliably changes the answer; in this regime the answers it changes between are
nearly equivalent. This is weaker than a same-sample regret statistic would suggest, which is why
that statistic was withdrawn.
...
\textbf{Low discrimination between methods.} At $B=6000$ on these instances the mitigated arms are
nearly tied per cell, so this design can establish that the defect changes selections but has little
power to show that such changes matter. A regime with better-separated methods would be needed for
that, and we do not claim our null result generalises to one.
...
\section{Conclusion}
Two integration errors in an error-mitigation pipeline changed which method a benchmark selected in
\FlipRateAllPct\% of cells, without producing any error or implausible output --- though in this
regime the competing methods are so close that the measured held-out cost of the wrong selection is
about an order of magnitude below our declared practical threshold. Neither hazard is new, and the standard tooling
