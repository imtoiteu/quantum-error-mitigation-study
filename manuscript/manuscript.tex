...
\title{A Silent Measurement-Mapping Defect in Zero-Noise-Extrapolation\\
Pipelines Changes Which Error-Mitigation Method Appears Best}
...
the headline conclusion did not survive correction; we retract it. We release the corrected
implementation, the regression tests, the prospectively fixed protocol, and all raw data.
...
\item We \textbf{retract} the affected conclusion of our own earlier frozen study and release the
      corrected artifact (Section~\ref{sec:retraction}).
\end{itemize}
...
\begin{table}[t]
\caption{Realised cost per estimate by method, corrected implementation.}
\label{tab:cost}
\centering\small
...
frozen study, which we retract. We recommend that mitigation pipelines assert the measurement map
after any circuit rewrite, and that readout calibration be indexed by the physical qubits the
classical bits actually read.
