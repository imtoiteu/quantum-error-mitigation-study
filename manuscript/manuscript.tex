...
\textbf{not} a proven global ansatz optimum; ``ratio'' is $\langle C\rangle/C_{\max}$; ``at best''
is the fraction of 60 restarts reaching that value.}
\label{tab:instances}
\centering\small
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.86\textwidth]{../figures/fig0_pipeline}
\caption{The budget-matched ZNE/REM pipeline and the two silent defect sites. Neither raises an
error: the pipeline returns a well-formed circuit and a plausible expectation value. The oracle at
lower right detects both without any noise model.}
\label{fig:pipeline}
\end{figure*}
...
($B=6000$, six instances $\times$ 30 seeds, $n=180$ pairs per row). $D$ is the absolute difference in
estimation error, in expected cut value. The signed mean is far smaller than the mean $|D|$: the
defect randomises results rather than biasing them in one direction.
...
defective and corrected implementations. \emph{Regret} is the excess true error (measured under the
corrected implementation) from following the defective recommendation, averaged over flipped cells;
\emph{best} is the mean true error of the correct choice.}
\label{tab:flip}
\centering\footnotesize
\input{tables/tab_flip}
\end{table}
...
significant after Benjamini--Hochberg correction (largest adjusted $p$ was \FONemaxP), with mean
...
on (instance, seed), $n=180$ per row. $\Delta$ is the mean paired difference in estimation error
(negative favours the mitigated arm); $\Delta/C_{\max}$ expresses it in approximation-ratio units.
$p_{\mathrm{BH}}$ is Benjamini--Hochberg adjusted within the \FTwoNTests-test family.
Verdicts: ``helps''/``harms'' denote a statistically resolvable effect; an asterisk$^{\ast}$ marks
effects at or above the declared practical threshold of $0.01$ in approximation-ratio units;
``---'' denotes no resolvable difference.}
\label{tab:f2}
\centering\small
\input{tables/tab_f2}
\end{table*}
